	...
	while (TRUE)
	{
		...
		else
		{
			if (renderiza)
			{
				...
				{
					if (gamPad->GetState().Gamepad.sThumbLY > XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE) {
						scene->pxAnterior = scene->px;
						scene->pzAnterior = scene->pz;
						scene->move('f');
						renderiza = true;
					}
					if (gamPad->GetState().Gamepad.sThumbLY < -XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE) {
					...
					if (gamPad->GetState().Gamepad.sThumbLX > XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE) {
						scene->pxAnterior = scene->px;
						scene->pzAnterior = scene->pz;
						scene->move('r');
						renderiza = true;
					...
					//camara 
					if (gamPad->GetState().Gamepad.sThumbRY > XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE)
						scene->turnUp();
					if (gamPad->GetState().Gamepad.sThumbRY < -XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE)
						scene->turnDown();
					if (gamPad->GetState().Gamepad.sThumbRX > XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE)
						scene->turnRight();
					if (gamPad->GetState().Gamepad.sThumbRX < -XINPUT_GAMEPAD_LEFT_THUMB_DEADZONE)
						scene->turnLeft();
				}
